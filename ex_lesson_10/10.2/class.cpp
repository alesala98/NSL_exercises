...
Generation :: Generation(unsigned int nc, unsigned int nind, int rank, Random r){
...
vector<int> Generation :: Gen_Individual(){
	...
	srand(time(0)+m_rank);				// Shuffle of initial cities is done using different
    	random_shuffle(ind.begin()+1, ind.end());	// seeds, i.e. using the rank of the node
...
void Generation :: Print_L(vector<vector<int>> inds, vector<City> cities, unsigned int ngen, int rank){

	ofstream Ave_L1, Best_L1;
	Ave_L1.open("../Datas/ave_L1_"+to_string(rank)+".dat", ios::app);
	Best_L1.open("../Datas/best_L1_"+to_string(rank)+".dat", ios::app);
...
void Generation :: Print_Path(vector<vector<int>> inds, vector<City> cities, int rank){

	ofstream Path;
	Path.open("../Datas/path_"+to_string(rank)+".dat", ios::app);
...
void Init(Random rnd, int rank){
	...
	int seed[4];
   	...
   	if(input.is_open()){
      		...
      		input.close();
  	} else cerr << "Unable to open seed.in" << endl;
