...
int main(){ 
  ...
  //Equilibration();			//Once the equilibration is done you shouldn't use this function anymore -> commented

  for(unsigned int iblk=1; iblk<=nblock; ++iblk){
...
void Input(void){ 		//Prepare all stuff for the simulation
  ...
  if(restart==false){						//Check if file 'config.0' has been already modified
...
void Reset(unsigned int iblk){ 		//Reset block averages
...
void ConfFinal(void){ 		//Write final configuration
  ...
  cout << endl << "Print final configuration time t to file config.final and time t-dt to file old.final" << endl << endl;
  ...
  if(restart==false){
	...
	ofstream RewriteInput;
	RewriteInput.open("input.dat");

	RewriteInput << temp << endl << npart << endl << rho << endl << rcut << endl;
	RewriteInput << delta << endl << nstep << endl << nblock << endl << restart;
...
void Averages(unsigned int iblk){

  double r, gdir;

  cout << "Block number " << iblk << endl;

  ofstream AveEpot, AveEkin, AveTemp, AveEtot, Gave;
  ...
  for(int i=igofr; i<nbins+igofr; ++i){
	...
	r = (i-igofr) * bin_size;
       	gdir = (blk_av[i]/blk_norm)/(rho*(double)npart*(4.*M_PI/3.)*(pow(r+bin_size,3)-pow(r,3)));			//g(r)
...
void Equilibration(){
  ...
  cout << endl << "Equilibration of the system..." << endl << endl;
...
/*void ConfXYZ(unsigned int nconf){ 	//Write configuration in .xyz format
...
double Error(double sum, double sum2, unsigned int iblk){
