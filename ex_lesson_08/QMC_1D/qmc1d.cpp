...
double variationalWaveFunction_second(double x)
{
	...
	double mu = 0.81, sigma = 0.62;
	return  exp(-pow(x-mu,2.)/(2.*sigma*sigma))*(pow((x-mu)/(sigma*sigma),2.)-(1./(sigma*sigma))) +				
...
{
	ofstream out("../Datas/potential.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out("../Datas/kinetic.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out("../Datas/probability.dat");
